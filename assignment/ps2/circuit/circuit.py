...
class PriorityQueue:
    ...
    def _decrease_key(self, idx, key):
        if key is None:
            raise ValueError('Cannot increase to None in the queue')
        if (key > self.queue[idx]):
            raise ValueError('you can only decrease key')
        self.queue[idx] = key
        p_idx = self._parent(idx)
        while idx > 0 and self.queue[p_idx] > self.queue[idx]:
            self.queue[idx], self.queue[p_idx] =  self.queue[p_idx], self.queue[idx]
            idx = p_idx
            p_idx = self._parent(idx)

    def _heapify(self, idx):
        l, r = self._left(idx), self._right(idx)
        if l < len(self.queue) and self.queue[l] < self.queue[idx]: # find out the smallest between r, l and idx
            min_idx = l
        else:
            min_idx = idx
        if r < len(self.queue) and self.queue[r] < self.queue[min_idx]:
            min_idx = r
        if not (min_idx == idx):
            self.queue[idx], self.queue[min_idx] = self.queue[min_idx], self.queue[idx]
